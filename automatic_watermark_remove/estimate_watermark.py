	...
	for r, dirs, files in os.walk(foldername):
		...
		for file in files:
			...
			if img is not None:
				# 统一图像尺寸
				# images.append(img)
				images.append(img[80:220, 1:300])	# 截取有用图像
			else:
				print("    %s not found."%(file))

	# Compute gradients
	print("    Computing gradients.")
	grdsp = list(map(lambda x: cv2.Sobel(x, cv2.CV_64F, 1, 0, ksize=KERNEL_SIZE), images))
	grady = list(map(lambda x: cv2.Sobel(x, cv2.CV_64F, 0, 1, ksize=KERNEL_SIZE), images))
	# print('len_grdsp: ', np.array(grdsp).shape)
	# print('len_grady: ', np.array(grdsp))

	# Compute median of grads
	print("    Computing median gradients.")
	Wm_x = np.median(np.array(grdsp), axis=0)
	Wm_y = np.median(np.array(grady), axis=0)

	return (Wm_x, Wm_y, grdsp, grady)
...
def poisson_reconstruct2(grdsp, grady, boundarysrc):
...
def poisson_reconstruct(grdsp, grady, kernel_size=KERNEL_SIZE, num_iters=100, h=0.1, 
		boundary_image=None, boundary_zero=True):
	"""
	Iterative algorithm for Poisson reconstruction. 
	Given the grdsp and grady values, find laplacian, and solve for image
	Also return the squared difference of every step.
	h = convergence rate
	"""
	fxx = cv2.Sobel(grdsp, cv2.CV_64F, 1, 0, ksize=kernel_size)
...
def crop_watermark(grdsp, grady, threshold=0.4, boundary_size=2):
	...
	return grdsp[xm:xM, ym:yM, :] , grady[xm:xM, ym:yM, :]
...
def watermark_detector(img, gx, gy, thresh_low=200, thresh_high=220, printval=False):
	...
	# print('    img_edgemap: ', type(img_edgemap))
	...
	x = 147
	y = 84
	cv2.rectangle(im, (y, x), (y+sizew, x+sizeh), (255, 0, 0))
	return (im, (x, y), (sizeh, sizew))
