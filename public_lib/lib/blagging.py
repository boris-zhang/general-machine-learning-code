...
def _parallel_build_balanced_estimators(n_estimators, ensemble, X, y,
    ...
    for i in range(n_estimators):
        ...
        sample_counts = np.bincount(indices, minlength=n_samples)   #updated by zhangzhiyong@2018-04-19

        # TEF: Main change in this next call to _downsample
        (Xbal, ybal) = _downsample((X[indices])[:, features], y[indices])
        print('Xbal.shape, sum(ybal): ', Xbal.shape, np.sum(ybal))
...
def _downsample(X, y):
        # X = np.array(X)
        # X = X.toarray()            #updated by guojiahao@2018-06-15
        X = sparse.csr_matrix(X)      #updated by guojiahao@2018-06-15
        y = np.array(y)
        # print('y: ', y)
        ...
        indices = np.array(range(X.shape[0]))           #updated by guojiahao@2018-06-15
        ...
        majority_egs_idx = majority_egs_idx[:n_minority * n_negsamp]        #updated by guojiahao@2018-06-16
        total_egs_idx = np.append(minority_egs_idx, majority_egs_idx)
        # Extract examples of each class and combine
        X = X[total_egs_idx]
        y = y[total_egs_idx]
        X = sparse.coo_matrix(X)         #updated by guojiahao@2018-06-15
        return (X, y)
