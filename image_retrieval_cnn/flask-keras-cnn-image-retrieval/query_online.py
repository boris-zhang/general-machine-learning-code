...
print("\ntop %d images in order by dot are: " % maxres, imlist)
# print(scores)
...
rank_score = scores[rank_ID]
imlist = [imgNames[index] for i, index in enumerate(rank_ID[0:maxres])]
print("\ntop %d images in order by consine distance are: " % maxres, imlist)
print(np.linalg.norm(queryVec), np.linalg.norm(feats.T))


# show top #maxres retrieved result one by one
# for i, im in enumerate(imlist):
# 	print(args["result"]+"/"+im)
